...
__export(utils_exports, {
  ...
  clone: () => clone,
  createSizedTexture: () => createSizedTexture,
...
function createSizedTexture(img, blur) {
  ...
  if (adapter) {
    for (const [, p] of [["_", adapter], ...Object.entries(adapter)]) {
      if (p.prototype instanceof AbstractAdapter) {
        checkVersion(p.id, p.VERSION, "5.15.0");
        return p;
      }
    }
  }
  ...
  createMesh(panoData) {
    if (this.config.shader) {
      ...
      const material = new ShaderMaterial({
        uniforms,
        vertexShader: equirectangular_vertex_default,
        fragmentShader: equirectangular_fragment_default,
        depthTest: false,
        depthWrite: false,
        transparent: true
      });
...
_EquirectangularAdapter.VERSION = "5.15.0";
...
_DualFisheyeAdapter.VERSION = "5.15.0";
...
var CONFIG_PARSERS = {
  ...
  fisheye: (fisheye) => {
    ...
    return MathUtils5.clamp(fisheye, 0, 2);
  },
  ...
  cleanPanoramaOptions(options, panoData) {
    ...
    if (isNil(options.position) && !isNil(panoData.initialHeading) && !isNil(panoData.initialPitch)) {
      options = {
        ...options,
        position: {
          yaw: MathUtils7.degToRad(panoData.initialHeading),
          pitch: MathUtils7.degToRad(panoData.initialPitch)
        }
      };
    }
    return options;
...
var Renderer = class extends AbstractService {
  ...
  transition(textureData, options, transition) {
    ...
    const warmupTarget = new WebGLRenderTarget();
...
var Viewer = class extends TypedEventTarget {
  ...
  setPanorama(path, options = {}) {
    ...
    if (options.sphereCorrection === void 0) {
