/*!
 * Photo Sphere Viewer / Map Plugin 5.15.0
...
var MapComponent = class extends AbstractComponent3 {
  ...
  __onKeyPress(e) {
    ...
    let y = 0;
    let z = 0;
    if (e.matches(CONSTANTS2.KEY_CODES.ArrowUp)) y = 1;
    if (e.matches(CONSTANTS2.KEY_CODES.ArrowDown)) y = -1;
    if (e.matches(CONSTANTS2.KEY_CODES.ArrowLeft)) x = 1;
    if (e.matches(CONSTANTS2.KEY_CODES.ArrowRight)) x = -1;
    if (e.matches(CONSTANTS2.KEY_CODES.Plus)) z = 1;
    if (e.matches(CONSTANTS2.KEY_CODES.Minus)) z = -1;
    if (e.matches(CONSTANTS2.KEY_CODES.PageUp)) z = 1;
    if (e.matches(CONSTANTS2.KEY_CODES.PageDown)) z = -1;
    if (x || y) {
      this.state.mouseX = 0;
      this.state.mouseY = 0;
...
var _MapPlugin = class _MapPlugin extends AbstractConfigurablePlugin {
